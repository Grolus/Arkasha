...
from ..models import ClassBase, SubjectBase, LessonBase, HomeworkBase, GroupNumberEnum
...
class ClassDAO(BaseDAO):
    ...
    @classmethod
    async def get_lessons(cls, class_: ClassBase) -> Timetable:
        return class_.lessons
    ...
    async def insert_timetable(cls, session: AsyncSession, class_id: int, timetable: Timetable):
        ...
        for wd, lessons in timetable.timetable_dict.items():
            for pos, lesson in enumerate(lessons):
                if len(lesson.subjects) == 1:
                    subject_name = lesson.subjects[0].name
                ...
                elif len(lesson.subjects) == 2:
                    for subject, group in zip(lesson.subjects, [GroupNumberEnum.FIRST, GroupNumberEnum.SECOND]):
